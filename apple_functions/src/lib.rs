...
pub mod external {
  ...
  mod serde_str_array {
        ...
        S: Serializer
    {
        unsafe {
          ...
          let mut ser_tuple = serializer.serialize_str(&String::from_utf8_lossy(unsigned));
          ser_tuple
      }
    }
  ...
  pub struct MachineInfo
  {
      ...
      #[serde(with = "serde_str_array")]
      pub platform_serial: [i8; 64],

      // platform uuid [uuid4]
      #[serde(with = "serde_str_array")]
      pub platform_uuid: [i8; 64],
...
macro_rules! check_len_valid {
    ($self:ident, $x:ident, $max_len:literal) => {
        if $self.$x.len() == 0 || $self.$x.len() >= $max_len {
...
impl RequiredSystemFields {
  pub fn check_error(&self) -> Option<FieldLengthError> {
    ...
    check_len_valid!(self, platform_serial, 37);
    check_len_valid!(self, platform_uuid, 37);
    check_len_valid!(self, mlb, 37);
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
      match &self.kind {
        ...
        IDSErrorType::InvalidJsonField(err) => {
          f.write_fmt(format_args!("Json field {} has invalid length: {}", err.field, err.length))
        }
      }
    }
}
  ...
  fn create_machine_info(info: &RequiredSystemFields) -> Result<MachineInfo, NacError> {
    unsafe {
      ...
      build_machine_info::call(
        info.board_id.as_ptr() as *mut i8,
        info.root_disk_uuid.as_ptr() as *mut i8,
        info.product_name.as_ptr() as *mut i8,
        info.platform_serial.as_ptr() as *mut i8,
        info.platform_uuid.as_ptr() as *mut i8,
        info.mlb.as_ptr() as *mut i8,
        info.rom.as_ptr() as *mut i8,
        info.mac.as_ptr() as *mut i8,
...
fn test_validation() {
  ...
  unsafe {
    ...
    let result = init_nac_request::call(&cert, &machine, &mut context, &mut request)
    ...
    let result = sign_nac_request::call(context, &session_data, &mut validation_sig, &mut validation_len);
    ...
    let mut file = std::fs::OpenOptions::new()
      ...
      .write(true)
      .truncate(true)
      .open("C:\\Users\\hswek\\Documents\\SkPy-master\\pypush\\anisette\\request").unwrap();
    
    let b64 = base64::encode(data);

    file.write_all(b64.as_bytes()).expect("Failed to write b64");

    println!("{}", validation_len);
  }
}
...
fn gen_machine() {
  unsafe {
    ...
    let result = build_machine_info::call(
      board_id.as_ptr(),
       root_disk_uuid.as_ptr(),
       product_name.as_ptr(),
       io_platform_serial.as_ptr(),
